...
if [ ! -d "/hapsu" ]; then
    ...
    git clone git@github.com:sebastyijan-fi/hapsu.git /hapsu || { echo "Git clone failed"; exit 1; }
fi
...
pip install -r requirements.txt || { echo "Package installation failed"; exit 1; }

echo "Starting the bot with PM2..."
pm2 start bot.py --name hapsu || { echo "PM2 start failed"; exit 1; }
